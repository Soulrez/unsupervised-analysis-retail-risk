# This module only cleans and extracts the Risk Factor, # of transactions, Total items, and Total sales! (Baseline stats)
import xlrd
import csv
import re
...
def write_csv():
	...
	newcsv = open('data/safeway/report_minimal.csv', 'wb')
	...
	for sheet in sheets:	
		...
		for rownum in xrange(sh.nrows):
			wr.writerow(sh.row_values(rownum,start_colx=3, end_colx=7))
	newcsv.close()

def remove_top():
	# Get rid of the top blank lines
	with open('data/safeway/report_minimal.csv', 'r') as fin:
		data = fin.read().splitlines(True)
	with open('data/safeway/report_minimal.csv', 'w') as fout:
